...
export class MasterLobbyService {
  ...
  private tick = async () => {
    ...
    try {
      this.broadcastLobbies(lobbies);
    } catch (error) {
      this.log.error("Error broadcasting lobbies:", error);
    }

    try {
      await this.maybeScheduleLobby(lobbies);
    } catch (e) {
      this.log.error("Error in lobby scheduling:", e);
    }
  };

  private broadcastLobbies(games: PublicGameInfo[]) {
    const msg = {
  ...
  private async maybeScheduleLobby(lobbies: PublicGameInfo[]) {
    if (lobbies.length >= 2) {
      return;
    }
